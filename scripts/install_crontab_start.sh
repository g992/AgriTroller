...
CRON_CMD="cd ${PROJECT_ROOT} && AGRITROLLER_ENV=${ENV_NAME} AGRITROLLER_HOST=${HOST} AGRITROLLER_PORT=${PORT} AGRITROLLER_LOG_LEVEL=${LOG_LEVEL} AGRITROLLER_FRONTEND_DIST=${FRONTEND_DIST} /bin/bash ${STAT_SCRIPT} start >> ${LOG_FILE} 2>&1"
